...
// --- MOCK SUPABASE BACKEND (Step 1-3 Infrastructure) ---
const SupabaseEdgeFunctions = {
  fetchRegime: async (market) => {
    return new Promise((resolve, reject) => {
      // 5% chance of simulated API failure for graceful error handling
      if (Math.random() < 0.05) return setTimeout(() => reject(new Error("Data Feed Timeout")), 800);
      ...
      setTimeout(() => resolve({
        ...
        macroNote: market === 'Tadawul' 
          ? "TASI holding 12k support. Oil volatility driving rotation into financials."
          : "VIX expanded > 25. S&P breaking 50 EMA. Institutional distribution detected.",
      ...
      market === 'Tadawul' ? [
        {
          ...
          aiContext: 'Tight consolidation above rising 50 EMA with improving relative strength versus sector peers.'
        },
        {
          ...
          aiContext: 'Holding critical institutional support on light volume. Mean reversion probability high.'
        }
      ] : []
    ), 1200));
  },
...
export default function TradingOS() {
  ...
  useEffect(() => {
    let isMounted = true;
    setSystemStatus({ state: 'loading', message: 'Querying Data Feeds...' });
    ...
    const loadData = async () => {
      try {
        ...
        if (isMounted) {
          setSystemStatus({ state: 'degraded', message: 'Data Feed Delayed. Using fallback state.' });
  ...
  const executionState = useMemo(() => {
    const saudiFormatter = new Intl.DateTimeFormat('en-US', {
      timeZone: 'Asia/Riyadh', hour12: false, hour: 'numeric', minute: 'numeric', weekday: 'short'
    });
    ...
    <div className="fixed inset-0 bg-[#060608]/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <form onSubmit={handleJournalSubmit} className="bg-[#0b0d10] border border-slate-800 rounded-xl w-full max-w-lg overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <input required type="text" placeholder="e.g. 2222.SR" className="w-full bg-[#060608] border border-slate-800 rounded-lg px-4 py-2 text-slate-200 text-sm outline-none focus:border-indigo-500/50" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-2 block">Execution Quality</label>
              <select className="w-full bg-[#060608] border border-slate-800 rounded-lg px-4 py-2 text-slate-200 text-sm outline-none focus:border-indigo-500/50">
          ...
          <div>
            ...
            <textarea required rows={3} placeholder="Record structural logic and deviations from your rules." className="w-full bg-[#060608] border border-slate-800 rounded-lg px-4 py-3 text-slate-200 text-sm outline-none focus:border-indigo-500/50 resize-none" />
          </div>
        </div>
        <div className="p-5 bg-slate-900/20 border-t border-slate-800/60 flex justify-between items-center">
          ...
          <button type="submit" disabled={journalStatus !== 'idle'} className="flex items-center gap-2 bg-slate-100 hover:bg-white text-[#060608] disabled:opacity-50 px-6 py-2 rounded-md text-xs font-bold uppercase tracking-widest transition-all">
            {journalStatus === 'idle' ? <><Send size={14} /> Commit Record</> : journalStatus === 'submitting' ? <><Activity size={14} className="animate-spin" /> Committing...</> : <><CheckCircle2 size={14} /> Committed</>}
          </button>
        </div>
      </form>
    </div>
      ...
      <div className="bg-[#0b0d10] border border-slate-800 rounded-xl p-6 flex flex-col h-full hover:border-slate-700 transition-colors">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 uppercase tracking-widest">{setup.name}</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <div className="text-emerald-400 font-bold px-2 py-0.5 rounded text-[10px] border border-emerald-500/20 uppercase tracking-widest">Tier {setup.grade}</div>
        ...
        <div className="grid grid-cols-2 gap-px bg-slate-800 mb-6 border border-slate-800 rounded-lg overflow-hidden">
          <div className="space-y-1 bg-[#060608] p-4"><p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Zone</p><div className="font-mono text-sm text-blue-400">{setup.entryZone}</div></div>
          <div className="space-y-1 bg-[#060608] p-4"><p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Price</p><div className="font-mono text-sm text-slate-200">{setup.lastPrice.toFixed(2)}</div></div>
          <div className="space-y-1 bg-[#060608] p-4"><p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest flex justify-between">Stop <span>ATR: {setup.atr.toFixed(2)}</span></p><div className="font-mono text-sm text-rose-400">{setup.stopLoss.toFixed(2)}</div></div>
          <div className="space-y-1 bg-[#060608] p-4"><p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Target (R{rMultiple})</p><div className="font-mono text-sm text-emerald-400">{setup.target1.toFixed(2)}</div></div>
        </div>
        <div className="mb-6 flex-grow">
          <p className="text-[11px] text-slate-400 leading-relaxed font-medium pl-3 border-l border-indigo-500/50">
            <span className="text-indigo-400 font-bold uppercase tracking-widest block mb-1">AI Context</span>{setup.aiContext}
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-end gap-6">
            <div>
              ...
              <div className="flex bg-[#0b0d10] p-1 rounded border border-slate-800">
                <button onClick={() => setMarket('Tadawul')} className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded transition-all ${market === 'Tadawul' ? 'bg-slate-200 text-[#060608]' : 'text-slate-500 hover:text-slate-300'}`}>Tadawul (SAR)</button>
                <button onClick={() => setMarket('US')} className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded transition-all ${market === 'US' ? 'bg-slate-200 text-[#060608]' : 'text-slate-500 hover:text-slate-300'}`}>US Equities</button>
              </div>
            </div>
            <div className="text-right flex flex-col items-end">
              <p className="text-[9px] uppercase tracking-widest text-slate-500 mb-2 font-bold flex items-center gap-1.5"><Wallet size={12} /> Base Capital</p>
              <div className="flex items-center gap-2 bg-[#0b0d10] border border-slate-800 rounded focus-within:border-slate-500 transition-colors px-1">
                <input type="number" value={accountBalance} onChange={(e) => setAccountBalance(Number(e.target.value) || 0)} className="bg-transparent px-3 py-1.5 w-28 text-right text-slate-200 font-mono text-sm outline-none" />
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#0b0d10] p-5 rounded-xl border border-slate-800 flex flex-col justify-between">
                <div className="flex justify-between items-start mb-6">
                  <h2 className="text-[9px] font-bold uppercase tracking-widest text-slate-500 flex items-center gap-2"><Globe size={12} /> Market Regime</h2>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${regime?.riskMultiplier > 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'}`}>Risk: {regime?.riskMultiplier}x</span>
                </div>
                <div className="mb-6"><p className="text-2xl font-bold text-slate-100 tracking-tight mb-1">{regime?.status}</p><p className="text-xs text-slate-500 font-medium">{regime?.environment}</p></div>
                <div className="mt-auto"><div className={`text-[10px] px-3 py-2 rounded font-bold uppercase tracking-widest border flex items-center gap-2 ${regime?.riskMultiplier === 0 ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}><AlertTriangle size={12} /> {regime?.action}</div></div>
              </div>

              <div className={`p-5 rounded-xl border flex flex-col justify-between transition-colors duration-700 ${executionState.color}`}>
                <div className="flex justify-between items-center mb-6"><h2 className="text-[9px] font-bold uppercase tracking-widest opacity-60 flex items-center gap-2"><Clock size={12} /> Execution Window</h2></div>
                <div className="mb-6"><p className="text-2xl font-bold mb-1 tracking-tight">{executionState.status}</p><p className="text-xs opacity-80 font-medium">{executionState.action}</p></div>
                <div className="mt-auto text-[10px] font-mono opacity-60 flex justify-between items-center bg-black/20 p-2 rounded"><span className="uppercase tracking-widest">AST Time</span><span>{new Intl.DateTimeFormat('en-US', { timeZone: 'Asia/Riyadh', hour: '2-digit', minute:'2-digit', second:'2-digit', hour12: false }).format(currentTime)}</span></div>
              </div>

              <div className="bg-[#0b0d10] p-5 rounded-xl border border-slate-800 flex flex-col justify-between">
                <div className="flex justify-between items-start mb-6"><h2 className="text-[9px] font-bold uppercase tracking-widest text-slate-500 flex items-center gap-2"><ShieldAlert size={12} /> Risk Architecture</h2></div>
                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between"><span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Max Daily Loss</span><span className="text-xs font-mono text-slate-300">{(riskRules.maxDailyDrawdown * 100).toFixed(1)}%</span></div>
                  <div className="flex items-center justify-between"><span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Sector Exposure Cap</span><span className="text-xs font-mono text-slate-300">{(riskRules.maxSectorExposure * 100).toFixed(1)}%</span></div>
                  <div className="flex items-center justify-between border-t border-slate-800 pt-3"><span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Regime Risk</span><span className="text-sm font-mono font-bold text-emerald-400">{(riskRules.maxRiskPerTrade * (regime?.riskMultiplier || 1) * 100).toFixed(1)}%</span></div>
                </div>
                <button onClick={() => setIsJournalOpen(true)} className="mt-auto w-full bg-[#060608] hover:bg-slate-800 border border-slate-800 text-slate-300 px-4 py-2 rounded text-[10px] uppercase tracking-widest font-bold transition-colors flex items-center justify-center gap-2"><BookOpen size={14} /> Open Journal</button>
              </div>
            </div>

            <div className="pt-4">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-sm font-bold text-slate-100 uppercase tracking-widest flex items-center gap-2"><BarChart size={16} className="text-slate-500" /> Institutional Opportunities</h2>
                <div className="text-[9px] uppercase tracking-widest text-slate-500 font-bold flex items-center gap-2"><Server size={10}/> Edge Filtered</div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {regime?.riskMultiplier === 0 ? (
                  <div className="col-span-full py-20 text-center bg-[#0b0d10] rounded-xl border border-rose-900/30 flex flex-col items-center justify-center">
                    <div className="bg-rose-500/10 p-4 rounded-full mb-6"><AlertOctagon size={48} className="text-rose-500" /></div>
                    <h2 className="text-4xl font-black text-slate-100 mb-4 tracking-tighter">NO TRADE TODAY</h2>
                    <p className="text-sm text-slate-400 max-w-lg mx-auto font-medium leading-relaxed mb-6">Market regime indicates a highly unfavorable risk/reward environment. Capital preservation is strictly enforced.</p>
                    <div className="px-4 py-2 bg-[#060608] border border-slate-800 rounded text-[10px] font-mono text-slate-500 uppercase tracking-widest">Action Required: Retain cash balance.</div>
                  </div>
                ) : setups.length > 0 ? (
